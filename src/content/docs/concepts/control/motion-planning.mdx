...
<Aside title="In Practice">
  For AI-trained robot policies (ACT, VLA), motion planning is often replaced by end-to-end learned action prediction — the model directly outputs joint positions from camera observations. Classical motion planning is still used for structured tasks and safety constraints.
</Aside>
