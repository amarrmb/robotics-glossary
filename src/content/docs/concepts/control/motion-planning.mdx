...
NVIDIA's Isaac platform includes **cuMotion** for GPU-accelerated motion planning:

```python
from isaac_ros_cumotion import CumotionPlanner

# Initialize planner with robot URDF
planner = CumotionPlanner(
    robot_urdf="franka.urdf",
    world_model=collision_world,
)
...
trajectory = planner.plan(
    ...
    num_seeds=32,  # Parallel planning seeds
)
```
...
- Integrates with Isaac ROS and MoveIt
