...
**Example:** A 7B parameter model in bf16 (2 bytes per weight) on a Jetson Thor (~200 GB/s *effective* bandwidth — nominal is 273, but ~190–200 is what decode actually achieves; see the hardware table below):
...
```
Hardware: Jetson Thor, ~200 GB/s effective bandwidth (273 nominal), ~2000 TOPS (FP8)
...
A cross-platform footnote: on DGX Spark, the `lm_step` at FP8 was 70.0 ms — virtually identical to Thor's 70.2 ms. We originally read this as more evidence of overhead-bound behavior ("more bandwidth didn't help"), but it isn't a valid test: Thor and Spark have the **same** memory subsystem (256-bit LPDDR5X at 273 GB/s nominal), so a bandwidth-bound model would also show identical times. We've since measured both machines delivering ~190 GB/s effective with llama.cpp batch-1 decode — identical walls. The NVFP4 result above stands on its own as the evidence for the overhead regime; the cross-platform comparison merely confirms the two machines share one memory wall.
...
| Platform | Memory Bandwidth (nominal) | Measured effective* | Compute (FP8) |
|----------|-----------------|-----|---------------|
| Jetson Orin NX 16GB | 102 GB/s | — | ~100 TOPS |
| Jetson Orin AGX 64GB | 205 GB/s | — | ~275 TOPS |
| Jetson Thor 128GB | 273 GB/s | ~190 GB/s | ~2000 TOPS |
| DGX Spark 128GB | 273 GB/s | ~190 GB/s | — |
| RTX 4090 | 1008 GB/s | — | 660 TOPS |
| A100 80GB | 2039 GB/s | — | 624 TOPS |
| H100 SXM | 3350 GB/s | — | 1979 TOPS |

*Effective = what batch-1 LLM decode actually achieves (llama.cpp Q4_K_M, measured 2026-06: two model
sizes on both machines all converge on ~190 GB/s ≈ 70% of nominal). Where we haven't measured, assume
~70% of nominal for LPDDR platforms; HBM platforms typically achieve a higher fraction. **Use the
effective number when computing $R_1$** — it's the difference between a prediction that lands within
5% and one that's 30% optimistic.
...
- **GPU partitioning (MIG) moves $R_3$, not $R_1$.** A MIG slice keeps full memory bandwidth (on unified-memory Jetsons) but a fraction of the SMs — $R_3$ scales inversely with slice size while $R_1$ stays put. Measured on Thor (JetPack 7.2): the full 20-SM GPU sits almost exactly at $R_1 \approx R_3$ (the crossover), so any slice is compute-bound ($\tau/R_3 \approx 1.1$ on the 12-SM and 8-SM slices — near-linear SM scaling, only ~6% cross-slice interference). Corollary: on a slice, quantization helps via *compute* (fewer bytes to dequant-multiply), and the "batch-1 is always bandwidth-bound" rule above no longer applies.

---

<CardGrid>
  <LinkCard
    title="Read the Full Paper (Zenodo preprint)"
    description="The Latency Floor Model: Predictive Performance Diagnostics for Real-Time AI Inference on Edge Accelerators — DOI 10.5281/zenodo.18840846"
    href="https://zenodo.org/records/18840846"
  />
  <LinkCard
